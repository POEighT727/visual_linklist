...
{
    int maxline=LINES-1,maxcol=COLS-1;
    mvaddch(maxline-1,maxcol,' ');
    for(int i=0;i<=maxcol;i++)mvaddch(maxline,i,' ');
    move(maxline-1,maxcol);
    refresh();
}
...
void Oper(LinkList *l,char op)
{
    ...
    switch(op)
    {
        case 'l':
            ...
            scanf("%d",&len);
            Clr();
            mvaddstr(maxline,0,"please enter the bits of number(0 means the bits are random)");
            refresh();
            scanf("%d",&bits);
        ...
        case 'a':
            ...
            getyx(stdscr,y,x);
            mvaddstr(maxline,0,"append ");
            printw("%d",val);
            getyx(stdscr,ty,tx);
            mvaddstr(ty,tx,",press any key to continue.");
            refresh();
            getch();
            move(y,x);
            break;
        ...
        case 'd':
            ...
            Clr();
            Display_LinkList(l,0);
            if(LinkList_Empty(l))break;
            getyx(stdscr,y,x);
            mvaddstr(maxline,0,"delete ");
            printw("%d",val);
            getyx(stdscr,ty,tx);
            mvaddstr(ty,tx,",press any key to continue.");
            refresh();
        ...
        case 'f':
            ...
            {
                mvaddstr(maxline,0,"find the value ");
                printw("%d",val1);
                getyx(stdscr,y,x);
                mvaddstr(y,x,",the node`s index is ");
                getyx(stdscr,y,x);
                printw("%d",val2);
